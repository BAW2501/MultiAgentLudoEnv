...
GOAL_TAILES = [[7, 7], [7, 8], [7, 9], [8, 7], [8, 8], [8, 9], [9, 7], [9, 8], [9, 9]]
...
PLAYER_1_HOME_TAILES = [[12, 3], [14, 3], [12, 5], [14, 5]]

PLAYER_2_GOAL_AREAL = [[7, 2], [8, 2], [8, 3], [8, 4], [8, 5], [8, 6]]

PLAYER_2_HOME_TAILES = [[5, 2], [5, 4], [3, 4], [3, 2]]

PLAYER_3_GOAL_AREAL = [[2, 9], [2, 8], [3, 8], [4, 8], [5, 8], [6, 8]]

PLAYER_3_HOME_TAILES = [[2, 11], [2, 13], [4, 11], [4, 13]]

PLAYER_4_GOAL_AREAL = [[9, 14], [8, 14], [8, 13], [8, 12], [8, 11], [8, 10]]

PLAYER_4_HOME_TAILES = [[11, 14], [13, 14], [13, 12], [11, 12]]

STARS_TAILES = [[15, 8], [10, 7], [8, 1], [7, 6], [1, 8], [6, 9], [8, 15], [9, 10]]

GLOBS_TIALES = [[13, 9], [14, 7], [9, 3], [7, 2], [3, 7], [2, 9], [7, 13], [9, 14]]
FREE_TAIALES = [
    [10, 9],
    [11, 9],
    [12, 9],
...
HOME_TAILES = {
    ...
    'red': PLAYER_1_HOME_TAILES,
    'green': PLAYER_2_HOME_TAILES,
    'blue': PLAYER_3_HOME_TAILES,
    'yellow': PLAYER_4_HOME_TAILES,
}

TRACKS = {0: TRACK_PLAYER_1, 1: TRACK_PLAYER_2, 2: TRACK_PLAYER_3, 3: TRACK_PLAYER_4,
          'red': TRACK_PLAYER_1, 'green': TRACK_PLAYER_2, 'blue': TRACK_PLAYER_3, 'yellow': TRACK_PLAYER_4}
...
PLAYER_COLORS = {
    ...
    3: PLAYER_4_COLOR,
    'red': PLAYER_1_COLOR,
    'green': PLAYER_2_COLOR,
    'blue': PLAYER_3_COLOR,
    'yellow': PLAYER_4_COLOR
}
...
def get_taile_cord(n, m):
    ...
    center = (
        top_left[0] + int(np.round((top_right[0] - top_left[0]) / 2)),
...
def draw_dice_backgound(board):
    ...
    draw_text(
        board, "Dice:", (bot_right[0], center[1]), (0, 0, 0), fontScale=1, thickness=2
    )
...
def draw_move_count_backgound(board):
    ...
    draw_text(
        board, "Round:", (bot_right[0], center[1]), (0, 0, 0), fontScale=1, thickness=2
    )
...
def put_image_at_taile(board, image, n, m, mask=None):
    ...
    board[
        center[1] - image_hight // 2 : center[1] + image_hight // 2,
        center[0] - image_width // 2 : center[0] + image_width // 2,
    ][mask] = image[mask]
...
class LudoVisualizer:
    def draw_basic_board(self):
        ...
        draw_multi_box(
            board, (7, 7), (9, 9), line_color=(0, 0, 0), fill_color=GOAL_COLOR
        )

        for n, m in STARS_TAILES + GLOBS_TIALES + FREE_TAIALES:
            draw_tail(
                board, n, m, line_color=(0, 0, 0), fill_color=TAILE_BACKGROUND_COLOR
            )

        for n, m in PLAYER_1_HOME_TAILES + PLAYER_1_GOAL_AREAL:
            draw_tail(
                board, n, m, line_color=(0, 0, 0), fill_color=PLAYER_1_AREAL_COLOR
            )

        for n, m in PLAYER_2_HOME_TAILES + PLAYER_2_GOAL_AREAL:
            draw_tail(
                board, n, m, line_color=(0, 0, 0), fill_color=PLAYER_2_AREAL_COLOR
            )

        for n, m in PLAYER_3_HOME_TAILES + PLAYER_3_GOAL_AREAL:
            draw_tail(
                board, n, m, line_color=(0, 0, 0), fill_color=PLAYER_3_AREAL_COLOR
            )

        for n, m in PLAYER_4_HOME_TAILES + PLAYER_4_GOAL_AREAL:
            draw_tail(
                board, n, m, line_color=(0, 0, 0), fill_color=PLAYER_4_AREAL_COLOR
            )

        return board

    def render_game_state(self, pieces, dice, players_dice, round_i):
        board = self.draw_basic_board()
        draw_moment(board, (pieces, dice, players_dice, round_i))
        return board
